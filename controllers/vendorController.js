...
export const getVendorWithDetails = async (req, res) => {
  try {
    ...
    // vendor.routes থেকে routeName array নাও
    const routeNames = vendor.routes.map(r => r.routeName).filter(Boolean);
    const routeIds = vendor.routes.map(r => r.routeId).filter(Boolean);

    // দুটো field-ই try করো (safety)
